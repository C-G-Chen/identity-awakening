...
import { Shuffle, ChevronDown, CheckCircle2, Eye } from 'lucide-react'
...
function SceneCard({ scene, isOld, isSelected, onSelect }: { scene: Scene; isOld: boolean; isSelected: boolean; onSelect: () => void }) {
  const [imgError, setImgError] = useState(false)
  return (
    <div
      onClick={onSelect}
      className={`relative rounded-2xl border overflow-hidden cursor-pointer transition-all duration-300 hover:scale-[1.02] ${
        isSelected
          ? isOld ? 'border-red-400/60 shadow-xl shadow-red-900/20 scale-[1.02]' : 'border-violet-400/60 shadow-xl shadow-violet-900/20 scale-[1.02]'
          : isOld ? 'border-red-500/20 hover:border-red-500/40' : 'border-violet-500/20 hover:border-violet-500/40'
      } bg-white/3`}
    >
...
function AntiVisionContent() {
  ...
  const [activeOldScene, setActiveOldScene] = useState(0)
  const [activeNewScene, setActiveNewScene] = useState(0)
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        {/* Old */}
        <div className={`rounded-3xl border overflow-hidden transition-all duration-500 ${antiVision.selectedSide === 'old' ? 'border-red-400/50 shadow-2xl shadow-red-900/20' : 'border-red-500/20'}`}>
          <div
            onClick={() => updateAntiVision({ selectedSide: 'old' })}
            className={`p-5 cursor-pointer transition-all duration-300 ${antiVision.selectedSide === 'old' ? 'bg-red-950/40' : 'bg-red-950/20 hover:bg-red-950/30'}`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-red-500/20 flex items-center justify-center text-lg">😔</div>
                <div>
                  <div className="text-red-400 text-xs font-semibold uppercase tracking-wider">当前身份</div>
                  <h3 className="text-white font-bold text-lg">{antiVision.oldIdentityLabel || '我现在是……'}</h3>
                </div>
              </div>
              {antiVision.selectedSide === 'old' && (
                <div className="flex items-center gap-1.5 bg-red-500/20 text-red-300 text-xs font-semibold px-3 py-1.5 rounded-full">
                  <Eye className="w-3 h-3" />正在审视
                </div>
              )}
            </div>
          </div>
          <div className="p-4 bg-background-DEFAULT/50 space-y-3">
            {oldIdentityScenes.map((scene, i) => (
              <SceneCard
                key={scene.id}
                scene={scene}
                isOld={true}
                isSelected={antiVision.selectedSide === 'old' && activeOldScene === i}
                onSelect={() => { setActiveOldScene(i); updateAntiVision({ selectedSide: 'old' }) }}
              />
            ))}
          </div>
        </div>
        ...
        {/* New */}
        <div className={`rounded-3xl border overflow-hidden transition-all duration-500 ${antiVision.selectedSide === 'new' ? 'border-violet-400/50 shadow-2xl shadow-violet-900/20' : 'border-violet-500/20'}`}>
          <div
            onClick={() => updateAntiVision({ selectedSide: 'new' })}
            className={`p-5 cursor-pointer transition-all duration-300 ${antiVision.selectedSide === 'new' ? 'bg-violet-950/40' : 'bg-violet-950/20 hover:bg-violet-950/30'}`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-violet-500/20 flex items-center justify-center text-lg">🚀</div>
                <div>
                  <div className="text-violet-400 text-xs font-semibold uppercase tracking-wider">渴望身份</div>
                  <h3 className="text-white font-bold text-lg">{antiVision.newIdentityLabel || '我想成为……'}</h3>
                </div>
              </div>
              {antiVision.selectedSide === 'new' && (
          ...
          <div className="p-4 bg-background-DEFAULT/50 space-y-3">
            {newIdentityScenes.map((scene, i) => (
              <SceneCard
                key={scene.id}
                scene={scene}
                isOld={false}
                isSelected={antiVision.selectedSide === 'new' && activeNewScene === i}
                onSelect={() => { setActiveNewScene(i); updateAntiVision({ selectedSide: 'new' }) }}
              />
            ))}
          </div>
        </div>
...
export default function AntiVisionBuilder() {
  const { data, updateAntiVision } = useIdentity()
  const { antiVision } = data
  const canComplete = !!antiVision.selectedSide || !!(antiVision.oldIdentityLabel && antiVision.newIdentityLabel)
  ...
  return (
    <StepWrapper
      ...
      subtitle="点击不同的生活场景，感受两种身份的真实重量。你正在保护哪一个？你真正渴望哪一个？"
      badgeLabel="身份觉醒"
      badgeColor="violet"
